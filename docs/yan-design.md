...
全系统命名的唯一来源，脚本、文档、`AGENTS.md` 都跟它对齐。
...
JSON 装不下「做到哪了」，而单独维护一份 `progress.md` 又必然跟各个 `outcome.md` 不一致。所以用 append-only 的一行式日志：
...
分工一句话：autoarm 负责起监督，guard 负责验证监督真的起来了。  一个做事，一个验收。
...
废弃的那一行 `log` 必须写清原因。 正常轮换的原因是显然的（上线了、有反馈）；废弃的原因才是最容易忘的—半年后看到 `2.0.3` 断在那儿，你会想不起来当初为什么扔掉它。
...
- **热复用（池存在的唯一理由）**：还树 = `reset --hard` + `clean -fd`，**永不带 `-x`**。
  `-x` 会连 gitignore 的东西一起删，那一个字母就是「秒级复用」和「每次冷装」的分界线
- **背压**：池满时 `get` **自己失败**，不再撑新树。这条和上一条是配套的——
  满了就新建的话池会慢慢长胖，多出来的全是冷树，等于没池
...
它是**磁盘和并行度的权衡**。默认给到 8 是因为并行度的上限不该由工具来替你设——
池满是个准确的信号，真撞上了你自己会知道。

需要往下调的信号有两个，都跟 monorepo 有关：
...
分支名属于决策那一类，而决策可以由外部权威做。yan 的职责只是「把决策记下来，并且不假设它长什么样」。`merge-check` 以后接进来是同一个道理：「能不能合」是决策，okt 可以是决策者，yan 只负责执行和记录。
...
一句话需求 → 一个 unit → 一个 shift → 子分支 MR 合回集成分支 → shift 下工还树 → 对外 MR 开出 → user 说合 → 合掉 → `log.md` 完整记录了整条链路。
