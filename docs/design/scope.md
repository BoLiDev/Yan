...
两个决定让范围比原稿大了一点：内置 worktree 池（[§7](worktree.md#7-worktree)）和 forge 层支持两个远端（[§8.4](delivery.md#84-forge-层lib-forgesh)）。
...
**定了：推。** 两级 MR 都保留，[§5.3](agents.md#53-shift-的生命周期) / [§6.1](branching.md#61-两级结构) / [§9.2](boundaries.md#92-外部副作用真正需要边界的部分) 维持当前写法。
...
wtpool 的 `--help` 标签 bug 那条（原 §12 的第 4 项）随 [§7](worktree.md#7-worktree) 内置池一起消失。
