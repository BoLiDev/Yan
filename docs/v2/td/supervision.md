...
- **Both harness bindings, unchanged.** Claude autoarm plus turn-end guard; Codex checkpoint loop plus guard. Herdr is the multiplexer axis; the harness is a different axis, and [td §5.6 / §5.7](../../mvp/td/agents.md#56-harness-requirements) keeps them apart.
  ...
  **The Codex half of that sentence was false until Phase 8.5, and this document was the main place it was asserted.** `.codex/hooks.json` was malformed, so codex warned once at startup and ran without it: `SessionStart` never called `yan session-start` and `Stop` never called the guard, in any session, ever. It has now been run — SessionStart reaches the rebuild, codex honours the blocking `Stop`, the budget spends and fails open, and `yan wait --seconds N` returns 124 on a quiet slice ([evidence §13.5](evidence.md#135-the-binding-run-once-end-to-end)). What that leaves is not a supervision question but a first-run one: [§7](#7-what-is-still-open) item 3.
...
3. **Plan approval arrives as `done`** ([§3](#done-does-not-mean-finished)). Whether more of Claude's and Codex's prompts miss is a manifest question, not a yan one — but `run/signal` is what covers the gap, and that is why it is not going anywhere.
   ...
   **Codex's prompts have now been enumerated, and one of them misses worse than that** ([evidence §13.3](evidence.md#133-which-of-codexs-prompts-herdr-recognises)). Codex's *"Hooks need review"* gate reports **`idle`**, with no rule matched at all — not `done`. The distinction is the whole difference between a degraded reason and no wake: a shift parked on that screen in an unfocused pane is invisible to every source supervision has, including `run/signal`, because the shift is not running yet and has nothing to report with. It is the one place where `run/signal` is *not* the cover, and it is why [§1](#1-what-was-inferred-and-what-is-now-known)'s "a missed `blocked` is a false negative, not a wrong answer" needs the qualifier *once the agent is running*.
