...
**The seam contract test was the migration's safety rail.** `tests/unit/lib-term-contract.test.sh` pinned what the seven functions must do; its assertions were ported to vitest in Phase 4 and had to pass against Herdr before tmux could be removed. Both halves ran side by side against different backends for five phases, which is the whole reason the contract was written down separately from either implementation. Phase 9 deleted the bash original with the backend it tested; `src/externals/herdr/terminal.test.ts` is what remains of it.
