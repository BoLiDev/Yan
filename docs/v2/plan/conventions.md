...
- **One error, and `code` says which condition.** `new YanError('worktree_mismatch', …)`. A subclass per module was tried and bought nothing: outside the module that throws, no caller ever distinguished one from another — every consumer asks `isYanError` and reads `message` and `exitCode`. Codes keep a module or command prefix (`worktree_mismatch`, `tree_usage`) because `code` outlives any class: it is what a test asserts on and what survives a JSON boundary. They are written out at the throw site rather than gathered in a table, so a code has one home.
- **It earns its place twice.** `isYanError(e)` is the ours-versus-theirs boundary — a Herdr or `gh` error object must never propagate — and `src/cli/shared/action.ts` needs one `instanceof` to map any of them to an exit code. Those are the only two reasons the class exists.
