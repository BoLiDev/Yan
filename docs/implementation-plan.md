...
| id | Delivers | Tests |
| --- | --- | --- |
| `yan-forge-github` | the `lib-forge.sh` interface definition plus the GitHub implementation (four verbs); dispatch by `forge` in `conf/config.json` | each of the four `forge_mr_state` values is produced at least once; **the return value can only ever be one of those four** (feed it a strange API response and it must return `unknown` rather than leaking the raw text); `forge_ci_state` reduces N check runs to one green or red; missing or invalid `forge` fails closed |
| `yan-forge-gitlab` | the GitLab implementation, including `forge.host` from `conf/config.json` | the same set of cases with a different fixture; missing `host` is refused. **Requires `glab` authenticated for that host, and a real GitLab target; see §4** |
