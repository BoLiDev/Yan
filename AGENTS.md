...
- You hold **no state of your own**. `yan session-start` rebuilds the whole picture from
  the task directories, the terminal, the worktree pool and the forge. Closing you and
  opening a new one costs nothing, so never write yourself a note to resume from.
...
## Authority: what needs `user` to ask
...
outbound merge request.** So the size of a unit is the size of a review a colleague can
get through in one sitting. When you can see that the finished diff would be too big for
that, it was two units.

- Two directories that will be released together and reviewed together: one unit.
- Two sub-applications of a monorepo: two units, even in one repository. You cannot
  change two applications on one branch.
...
**One shift, one piece of work.** If the brief needs the word "then" between two
unrelated things, it is two shifts.
...
get stuck with nobody noticing. `yan wait` is that watcher, and it has two shapes.

**On Claude Code** the Stop hook arms a long `yan wait` for you, so there is nothing to
remember. After a wake: `yan drain`, then act on the reason.

**On Codex there is no autoarm — the loop is yours to run**, and this paragraph is the
only thing standing between a Codex session and silent blindness (an interactive Codex
TUI may not fire the project's SessionStart hook at all, so do not assume it ran):
...
If the turn-end guard blocks you, the answer is another `yan wait --seconds` slice, not a
